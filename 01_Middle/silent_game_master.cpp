...
SilentGameMaster::SilentGameMaster(int board_size, int player_count)
	: board_size(board_size), strategy_pool(player_count), train_h1(std::make_shared<SimplePlayer>())
{}

void SilentGameMaster::initiate_players(int player_count)
{
	strategy_pool = std::vector<silent_strategy_wrapper>(player_count);
}

const SilentGameMaster::silent_strategy_wrapper& SilentGameMaster::get_winner()
{
	std::sort(strategy_pool.begin(), strategy_pool.end());
	return strategy_pool.front();
}

float SilentGameMaster::calculate_player_fittnes(const glm::dvec2 &score, int player, float secondary_score, float progress)
{
...
void SilentGameMaster::addSimpleGames(Updater &games)
{
...
void SilentGameMaster::addSimpleGames2(Updater &games)
{
	for (int s = 0; s < strategy_pool.size(); s++)
	{
		games.AddTask( /*LAMBDA START*/ [this, s](int iterations)->bool
...
void SilentGameMaster::addGames(Updater &games)
{
...
void SilentGameMaster::addSort(Updater &games)
{
	games.AddTask( /*LAMBDA START*/ [this](int iterations)->bool
	{
		...
		for (silent_strategy_wrapper &sw : strategy_pool)
		{
...
void SilentGameMaster::addCyborgWithBest(Updater &games)
{
...
void SilentGameMaster::save_matrix(const matrix &m, const std::string &filename) {
...
matrix SilentGameMaster::load_matrix(matrix &m, const std::string &filename) {
